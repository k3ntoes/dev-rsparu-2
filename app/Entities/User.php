...
class User extends MythUser
{
    ...
    public function getJwtHash()
    {
        $issuedAtTime = time();
        $tokenTimeToLive = getenv('app.sessionExpiration');
        ...
        $payload = array(
            ...
            "usernamae" => $this->attributes['username'],
            "iat" => time(),
            "nbf" => time(),
            'exp' => $tokenExpiration,
        );

        $this->jwtHash = JWT::encode($payload, getenv('encryption.key'));
        return $this->jwtHash;
    }
}
